...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_variables_can_also_be_used_to_access_captures
    ...
    assert_equal __, ::Regexp.last_match(1)
    assert_equal __, ::Regexp.last_match(2)
  end
  ...
  def test_sub_is_like_find_and_replace
    assert_equal __, 'one two-three'.sub(/(t\w*)/) { ::Regexp.last_match(1)[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal __, 'one two-three'.gsub(/(t\w*)/) { ::Regexp.last_match(1)[0, 1] }
  end
end
